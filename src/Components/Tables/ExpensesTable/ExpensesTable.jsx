...
const ExpensesTable = ({ tableList, pageHandler, page, totalCount, sumCount, filterDateHandler }) => {
  ...
  const columns = [
    ...
    columnHelper.accessor("parent", {
      cell: (info) => <span className={info.getValue() === null ? "font-semibold text-amber-500" : "font-semibold text-blue-400"}>{info.getValue() === null ? "رئيسي" : "فرعي"}</span>,
      header: "النوع",
    }),
    columnHelper.accessor("expense_type_description", {
      cell: (info) => <span>{info.getValue()}</span>,
      header: "الوصف",
    }),
    columnHelper.accessor("parent", {
  ...
  return (
    <div className="p-2 max-w-5xl mx-auto text-white fill-gray-400">
      <div className="flex justify-between mb-2">
        ...
        <DownloadBtn data={data} fileName="تصنيفات المصاريف" />
      </div>
              ...
              <tr key={getHeaderGroup.id}>
                {
                  getHeaderGroup.headers.map((header) => (
                    <th key={header.id} className="capitalize px-3.5 py-2 text-right">
                      {flexRender(
                ...
                `}>
                  {
                    row.getVisibleCells().map((cell) => (
                      <td key={cell.id} className={`px-3.5 py-2  text-right`}>
                        {flexRender(
