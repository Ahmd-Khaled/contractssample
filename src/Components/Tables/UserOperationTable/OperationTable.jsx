...
import FilterOperationsTable from "../FilterOperationsTable/FilterOperationsTable";


const OperationTable = ({ tableList, filterHandler, tableKey, from, to, amountKey, tableId, totalSum }) => {
  ...
  let columns = [
    ...
    columnHelper.accessor(tableId, {
      cell: (info) => <NavLink className="underline" to={`/contracts/${info.getValue()}`}>{info.getValue()}</NavLink>,
      header: "ID",
    }),
    columnHelper.accessor(amountKey, {
  ...
  return (
    <div className="p-2 max-w-5xl mx-auto text-white fill-gray-400">
      <div className="flex justify-between mb-2">
        ...
        <DownloadBtn data={data} fileName="عمليات المستخدم" />
      </div>
      <div className="w-full flex items-center justify-between gap-1  z-50 mb-2">
        <FilterOperationsTable 
          ...
          to={to}
        />
        <div className="p-1 px-2 py-1 bg-orange-400 text-white rounded-sm flex items-center gap-2">
          <span className="">الإجمالي:</span>
          <span className=" text-emerald-950 font-semibold text-lg">{totalSum}</span>
        </div>
      </div>
                ...
                `}>
                  {
                    row.getVisibleCells().map((cell) => (
                      ...
                      <td key={cell.id}  className={`px-3.5 py-2 `}>
                        {cell.getValue() 
                          === "deduction" ? <span className="text-red-400">خصم</span> 
                          : cell.getValue() === "bonus" ? <span className="text-green-400">حافز</span> 
