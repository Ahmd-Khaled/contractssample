...
const WithdrowalsTable = ({ tableList, pageHandler, page, totalCount, sumCount, filterDateHandler }) => {
  ...
  const columns = [
    ...
    columnHelper.accessor("withdraw_note", {
      cell: (info) => <span>{info.getValue()}</span>,
      header: "ملاحظة",
    }),
  ...
  return (
    <div className="p-2 max-w-5xl mx-auto text-white fill-gray-400">
      <div className="flex justify-between mb-2">
        ...
        <DownloadBtn data={data} fileName="عمليات السحب" />
      </div>
      <div className="w-full flex items-center justify-between gap-1  z-50 mb-2">
        ...
        {/* <div className="p-1 px-2 py-1 bg-red-400 text-white rounded-sm flex items-center gap-2">
          <span className="">إجمالي السحب:</span>
          <span className=" text-emerald-950 font-semibold text-lg">{sumCount}</span>
        </div> */}
      </div>
...
export default WithdrowalsTable
